...
func (d *Daemon) launchHubble() {
	...
	if address := option.Config.HubblePeerAddress; len(address) > 0 {
		var peerServiceOptions []serviceoption.Option
		if option.Config.HubbleTLSDisabled {
			logger.WithField("address", address).Warn("Hubble peer server will be exposing its API insecurely on this address")
			peerServiceOptions = append(peerServiceOptions, serviceoption.WithoutTLSInfo())
		}
		if peerSvc == nil {
			peerSvc = peer.NewService(d.nodeDiscovery.Manager, peerServiceOptions...)
		...
		if option.Config.HubbleTLSDisabled {
			...
			if err != nil {
				logger.WithError(err).Error("Failed to initialize Hubble peer server TLS configuration")
				return
			}
			waitingMsgTimeout := time.After(30 * time.Second)
			for tlsServerConfig == nil {
				...
				case tlsServerConfig = <-tlsServerConfigChan:
				case <-waitingMsgTimeout:
					logger.Info("Waiting for Hubble peer server TLS certificate and key files to be created")
				case <-d.ctx.Done():
					return
				}
			}
		...
		srv, err := server.NewServer(logger, options...)
		if err != nil {
			logger.WithError(err).Error("Failed to initialize Hubble peer server")
			if tlsServerConfig != nil {
				tlsServerConfig.Stop()
			}
			return
		...
		logger.WithField("address", address).Info("Starting Hubble peer server")
		if err := srv.Serve(); err != nil {
			logger.WithError(err).Error("Failed to start Hubble peer server")
			if tlsServerConfig != nil {
				tlsServerConfig.Stop()
			}
			return
